...
function balikanTanggalSql($tgl_sql)
{
  $tgl = explode("-", $tgl_sql);
...
{
  $tgl_mysql = substr($tgl_mysql, 0, 10);
...
function rupiah($nilai, $simbol = "Rp", $spasi_rupiah = "", $dibelakang_koma = 0, $penutup = "", $pemisah_koma = ",", $pemisah_ribuan = ".")
{
  ...
  return $simbol . $spasi_rupiah . number_format($nilai, 0, ".", $pemisah_koma) . $penutup;
}
...
{;
  // $penutup = ,-
  // $dibelakang_koma = 2 -> ,00
  return "<div style='float: left;'>" . $simbol . "</div> <div style='float: right;'>" . $spasi_rupiah . number_format($nilai, 0, ".", $pemisah_koma) . $penutup . "</div><div style='clear: both;'></div>";
}
...
// $user = level user sekarang
// $list_akses = daftar level user yang bisa mengakses halaman
...
// fungsi untuk membuat no kwitansi 
function noKwitansi($id_tgl_angsuran, $id_histori)
{
  // jika transaksinya bukan angsuran kredit, maka no kwitansi ditambah prefiks A
  // id_tgl_angsuran = NULL membuktikan bahwa transaksi tersebut bukan angsuran kredit
  if (empty($id_tgl_angsuran)) {
    return "A" . $id_histori;
  } else {
    return $id_tgl_angsuran;
  }
}
